 ...
 * -----------------------------------------------------------------
 * 重要說明：
 * EXP_TABLE（Lv.1~200）取自舊楓之谷（pre-Big Bang）流傳已久的經驗值表，
 * 跟 MapleStory Classic Worlds（國外已上線的楓之谷經典版）官方頁面比對，
 * Lv.1~49 完全一致，因此用同一張表延伸到 200 等。但「新楓之谷經典版」
 * 台版尚未上線，實際數值仍可能跟官方公告/封測結果有些微差異，等正式
 * 資料出來後請直接覆蓋這張表（其他計算邏輯完全不用改）。
 * 200 等以上目前沒有任何可信來源，故先不提供，避免顯示誤導數字。
 * -----------------------------------------------------------------
 */

// ===== 1. 等級所需經驗值表 =====
// index 0 = Lv.1 升 Lv.2 所需經驗，index 198 = Lv.199 升 Lv.200 所需經驗
const EXP_TABLE = [
