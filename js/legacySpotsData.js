...
const LEGACY_SPOTS = [
  ...
  },
  {
    ...
    entries: [
      ...
      { level: "15", monster: "綠菇菇", locations: "北部森林訓練場IV、南部森林訓練場III、弓箭手訓練場II&III、西部岩山II、墮落城市森林I&II、大木林III、菇菇山" },
      { level: "15", monster: "藍水靈", locations: "南部森林訓練場IV、北部森林訓練場V、墮落城市森林II、北方工地頂端、地鐵一號線（地區01）" },
      { level: "17", monster: "斧木妖", locations: "北部森林訓練場VI、西部岩石II、西部岩山III、幽深的峽谷I&II、東石岩石山III～V、野外之地" },
    ],
  },
  {
    levelRange: "19～20",
  ...
  },
  {
    ...
    entries: [
      ...
      { level: "22", monster: "刺菇菇", locations: "北部森林訓練場VII、藍菇菇森林、迷宮通道、黑森林系列、螞蟻洞I～IV、幽深螞蟻洞、螞蟻礦坑、弓箭手村迷宮入口、勇士之村迷宮入口、墮落城市森林III、黑暗通道、蝙蝠洞" },
      { level: "22", monster: "黑斧木妖", locations: "西部岩山III&IV、勇士之村迷宮入口、幽深的峽谷III、黑肥肥領土II、東石岩石山IV～VI、黑森林西入口、黑森林、野外之地" },
      { level: "23", monster: "木面怪人", locations: "遺跡發掘地I～III" },
      { level: "24", monster: "石面怪人", locations: "遺跡發掘地II&III" },
      { level: "24", monster: "黑肥肥", locations: "危險的峽谷I、火焰之地I、黑肥肥領土I&II、東石岩石山VI&VII、鋼之黑肥肥領土" },
      { level: "24", monster: "殭屍菇菇", locations: "弓箭手村迷宮入口、西部岩山IV、黑森林系列、螞蟻洞、蘑菇王之墓、黑暗通道、蝙蝠洞、幽深螞蟻洞I&II、螞蟻礦坑" },
      // 火獨眼獸實際等級是 27，引用來源（巴哈精華區）原文寫 025，是來源自己打錯——
      // 已用國際 pre-Big-Bang 資料庫（bbb.hidden-street.net）交叉驗證過 27 才對，
      // 之後校對別「修正」回 25
  ...
  },
  {
    ...
    entries: [
      ...
      // 洞穴幼年龍／幼龍保護者的巢穴名稱查到跟另一個資料庫（危險的龍之巢穴／
      // 遭破壞的龍之巢穴／主巢穴山峰）對不上，且查無 2007~2009 年代的直接
      // 記錄，兩隻的巢穴名稱先保留原樣但信心度較低，正式資料出來後要再核對
    ...
    note: "Lv.70~113 這段是米納爾森林（骷髏指揮官在鄰近的遺跡之峭壁），怪物等級已對照國際 pre-Big-Bang 資料庫驗證過，但查不到 2007~2009 年代台版的直接記錄，部分地圖細節（尤其兩隻幼龍的巢穴名稱）信心度較低。",
  },
];
 ...
 */
const LEGACY_BOSSES = [
  { level: "20", monster: "紅寶王", locations: "維多利亞 海岸叢林III（從維多利亞港一直往右走，約第 3～4 張地圖）", respawn: "無法查證（查到的來源可信度都不夠，沒有 2007～2009 年代能互相印證的直接記錄）" },
  { level: "35", monster: "樹妖王", locations: "勇士村 東方岩石山V", respawn: "約 3 小時（僅查到近年整理資料，2007～2009 年代記錄不足）" },
  { level: "38", monster: "仙人長老", locations: "納希沙漠 乾旱沙漠（地圖上方一帶，沒看到就換頻）", respawn: "3 小時" },
  ...
  { level: "77", monster: "喵怪仙人", locations: "桃花仙境 妖怪之林 → 妖怪之林2（進妖怪之林後點兩下頭上燈泡，由中間上方標示牌傳點進妖怪之林2，再從右上角隱藏傳點進入）", respawn: "2 小時 30 分～2 小時 50 分" },
  ...
  { level: "70", monster: "九尾妖狐", locations: "桃花仙境 狐狸山坡（左下方洞穴進入）", respawn: "3 小時" },
  ...
  { level: "110～120", monster: "海怒斯", locations: "水世界 海怒斯洞穴", respawn: "BB 大改版前（依 2009 年 PTT 玩家記錄，維修後重新計算）：右海怒斯約 32～48 小時、左海怒斯約 48～72 小時（重生時間官方調整過很多次，此為 BB 前的週期，跟現在版本的週期不同）" },
 ...
 * -----------------------------------------------------------------
 * 整理自巴哈姆特新楓之谷哈啦板／精華區玩家攻略，並與 PTT 楓之谷板封存
 * 文章、海外 BeforeBigBang 舊版資料庫交叉核對過。拉圖斯前置特別採用
 * 2009 年左右的舊文與海外舊版資料為準，避開近期改版過的
...
const LEGACY_BOSS_PREQUESTS = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        desc: "地點：不知名的廢礦、16區域，共需蒐集 7 把鑰匙才能取得火石母礦碎片。確切的箱子/礦石座標目前查無足夠可信的來源佐證，故意不列出，避免引用到不可靠資訊——實際跑的時候用地圖上的可互動物件逐一嘗試即可。另有 30 張廢礦卷可解，只是額外加經驗，不是前置必需項目。",
      },
      {
        title: "阿杜比斯的任務．第二階段",
        desc: "跳兩張忍耐（跳台）地圖，取得「火山的心臟」（部分資料寫作「火山的呼吸」，兩個名稱哪個正確目前查無法源確認）。這階段連原文攻略都表示沒有固定教學，只能自己跳。",
      },
      {
        title: "阿杜比斯的任務．第三階段（任務道具：火焰之眼）",
        desc: "打不知名的廢礦裡的挖礦殭屍（戴安全帽那種）收集 30 個金牙，連同火石母礦碎片＋上一階段拿到的道具一起交給阿杜比斯，完成後取得火焰之眼，即可進殘暴炎魔祭壇挑戰。",
      },
    ],
  },
  {
